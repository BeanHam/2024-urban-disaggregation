...
def train(model, 
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        ## error: taxi count/hour/city block
        loss = torch.sum(torch.abs(pred-source_high))/pred.size(0)/3751
...
def evaluation(model, 
    ...
    loss = torch.sum(torch.abs(pred_high-gt_high)).item()/pred_high.size(0)/3751
