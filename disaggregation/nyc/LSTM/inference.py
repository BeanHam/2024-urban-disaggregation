    ...
    for low, high in resolutions:
        ...
        for domain in domains:
            ...
            _,pred_high, gt_high = evaluation(model,
                                              ...
                                              dataset_test,
                                              parameters)
            pred_high = pred_high.reshape(pred_high.size(0)*pred_high.size(1),-1)
            gt_high = gt_high.reshape(gt_high.size(0)*gt_high.size(1),-1)
            ...
            if domain == data:
                ...
                np.save(f'../../../results/pred-gt/{data}/LSTM_{low}_{high}_pred.npy', pred_high)
                np.save(f'../../../results/pred-gt/{data}/LSTM_{low}_{high}_gt.npy', gt_high)
