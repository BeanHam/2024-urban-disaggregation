...
def main():
    ...
    loss_track_puma_rec = []
    loss_track_nta_rec = []
    loss_track_tract_rec = []
    loss_track_block_rec = []
    ...
    for epoch in tqdm(range(epochs)):
        ...
        loss_track_block.append(eval_results['loss_block'])
        loss_track_extreme.append(eval_results['loss_extreme'])        
        loss_track_puma_rec.append(eval_results['loss_puma_rec'])
        loss_track_nta_rec.append(eval_results['loss_nta_rec'])
        loss_track_tract_rec.append(eval_results['loss_tract_rec'])
        loss_track_block_rec.append(eval_results['loss_block_rec'])        
        loss = eval_results['loss_extreme']
        ...
        if es.save_model:            
            ...
            np.save(f'logs/block_loss.npy', loss_track_block)
            np.save(f'logs/extreme_loss.npy', loss_track_extreme)
            np.save(f'logs/puma_rec_loss.npy', loss_track_puma_rec)
            np.save(f'logs/nta_rec_loss.npy', loss_track_nta_rec)
            np.save(f'logs/tract_rec_loss.npy', loss_track_tract_rec)
            np.save(f'logs/block_rec_loss.npy', loss_track_block_rec)
