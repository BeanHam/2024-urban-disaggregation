...
def train(model, 
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        ## prediction
        puma, nta = model(source_low)
        loss_puma = torch.sum(torch.abs(puma-source_low))/puma.size(0)/3751
        loss_nta = torch.sum(torch.abs(nta-source_high))/puma.size(0)/3751
...
def evaluation(model, 
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        ## batch data
        indices = range(i, min(len(dataset), i+batch_size))        
        source_low, _ = zip(*[dataset[i] for i in indices])
        source_low = torch.stack(source_low).squeeze_(-1).to(device)
    ...
    gt_nta = dataset.source_high.squeeze_(-1)
