...
def load_data(low, high, parameters):
    ...
    ## prepare data
    dataset_train = taxi_data(X_low_train, X_high_train, X_nta_train, X_tract_train, chunk_size)
    dataset_val = taxi_data(X_low_val, X_high_val, X_nta_train, X_tract_train, chunk_size)
    dataset_test = taxi_data(X_low_test, X_high_test, X_nta_train, X_tract_train, chunk_size)
...
def train(model, 
    ...
    for i in range(0, len(dataset)-chunk_size, batch_size):
        ...
        source_low, source_high, source_nta, source_tract = zip(*[dataset[j] for j in indices])
        ...
        loss_tract = torch.sum(torch.abs(tract-source_tract))/puma.size(0)/chunk_size/3751
        loss_block = torch.sum(torch.abs(block-source_high))/puma.size(0)/chunk_size/3751
...
def evaluation(model, 
    ...
    for i in range(0, len(outer_indices), batch_size):
        ...
        source_low, _,_,_ = zip(*[dataset[outer_indices[j]] for j in inner_indices])
        source_low = torch.stack(source_low).squeeze_(-1).to(device)
    ...
    pred_block = torch.cat(pred_block).cpu()
    ...
    pred_block = pred_block.reshape(-1, pred_block.size(-1))
    ...
    rec_tract = rec_tract.reshape(rec_tract.size(0), -1, rec_tract.size(-1))
    
    gt_puma = dataset.source_low.squeeze_(-1)[:len(pred_puma)]
    gt_nta = dataset.source_nta.squeeze_(-1)[:len(pred_nta)]
    gt_tract = dataset.source_tract.squeeze_(-1)[:len(pred_tract)]
    gt_block = dataset.source_high.squeeze_(-1)[:len(pred_block)]
    ...
    loss_block = torch.sum(torch.abs(pred_block-gt_block)).item()/pred_block.size(0)/3751
    ...
    loss_tract_rec = 0
    for r in rec_tract:
        loss_tract_rec+=torch.sum(torch.abs(r-gt_tract)).item()/pred_tract.size(0)/3751
