...
def load_data(low_res_name, super_res_name, parameters):
    ...
    X_low = torch.from_numpy(np.load(att_low_res_path)).float()[:,index].unsqueeze_(-1)
    ...
    linkage = torch.from_numpy(np.load(linkage_path)).float()[index].unsqueeze_(0)
    ...
    ## train & test
    X_low_train = X_low[:int(0.8*len(X_low))]
    X_super_train = X_super[:int(0.8*len(X_low))]
    X_low_test = X_low[int(0.8*len(X_low)):]
    X_super_test = X_super[int(0.8*len(X_low)):]
    
    ## validation
    X_low_val = X_low_train[int(0.9*len(X_low_train)):]
    X_super_val = X_super_train[int(0.9*len(X_low_train)):]
    X_low_train = X_low_train[:int(0.9*len(X_low_train))]
    X_super_train = X_super_train[:int(0.9*len(X_super_train))]
