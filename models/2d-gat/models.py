...
class GraphSR(nn.Module):
    ...
    def __init__(self, linkage, super_adj, hops):
        super().__init__()
        ...
        nn.init.xavier_uniform_(self.lr3.weight, gain=0.2)
        ...
        self.v = nn.Parameter(torch.rand(self.super_size, 1))
        nn.init.xavier_uniform_(self.q.data, gain=0.01)
        nn.init.xavier_uniform_(self.k.data, gain=0.01)
        nn.init.xavier_uniform_(self.v.data, gain=0.01)
