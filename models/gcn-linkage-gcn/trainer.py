...
def main():
    ...
    model = GraphSR(dataset_train.adj_low.to(device), 
                    dataset_train.adj_super.to(device),
                    dataset_train.linkage.to(device),
    ...
    if log: 
        model.load_state_dict(torch.load(f'model_state/graphSR_{low_res_name}_{super_res_name}_{attention}'))
        loss_track = np.load(f'logs/loss_track_{low_res_name}_{super_res_name}_{attention}.npy')
    else:
        loss_track = []
    ...
    for epoch in tqdm(range(epochs)):
        ...
        torch.save(model.state_dict(), f'model_state/graphSR_{low_res_name}_{super_res_name}_{attention}')
        np.save(f'logs/loss_track_{low_res_name}_{super_res_name}_{attention}.npy', loss_track)
