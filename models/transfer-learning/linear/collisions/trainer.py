...
def main():
    ...
    for low_res_name, super_res_name in combinations(names, 2):
        ...
        for training_portion in training_portions:
            ...
            ## fine tune
            if fine_tune:
                model.load_state_dict(torch.load(f'model_state/graphSR_{low_res_name}_{super_res_name}_old'))
            ...
            for epoch in tqdm(range(epochs)):
                ...
                if loss < loss_min:
                    ...
                    torch.save(model.state_dict(), f'model_state/graphSR_{low_res_name}_{super_res_name}_{training_portion}_{fine_tune}')
                    np.save(f'logs/loss_track_{low_res_name}_{super_res_name}_{training_portion}_{fine_tune}.npy', loss_track)
