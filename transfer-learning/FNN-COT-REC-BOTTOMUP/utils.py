...
def load_data(low, high, parameters):
    ...
    ## load data
    X_puma_train = torch.from_numpy(np.load(source_puma_path+'_train.npy')).float()[-portion:]
    X_puma_test = torch.from_numpy(np.load(source_puma_path+'_test.npy')).float()
        
    X_nta_train = torch.from_numpy(np.load(source_nta_path+'_train.npy')).float()[-portion:]
    X_nta_test = torch.from_numpy(np.load(source_nta_path+'_test.npy')).float()    
    
    X_tract_train = torch.from_numpy(np.load(source_tract_path+'_train.npy')).float()[-portion:]
    X_tract_test = torch.from_numpy(np.load(source_tract_path+'_test.npy')).float()
    
    X_block_train = torch.from_numpy(np.load(source_block_path+'_train.npy')).float()[-portion:]
    X_block_test = torch.from_numpy(np.load(source_block_path+'_test.npy')).float()
    
    X_extreme_train = torch.from_numpy(np.load(source_extreme_path+'_train.npy')).float()[-portion:]
    X_extreme_test = torch.from_numpy(np.load(source_extreme_path+'_test.npy')).float()
    
    X_puma_train, X_puma_val, \
    X_nta_train, X_nta_val,\
    X_tract_train, X_tract_val, \
    X_block_train, X_block_val, \
    X_extreme_train, X_extreme_val = train_test_split(X_puma_train, 
                                                      X_nta_train, 
                                                      X_tract_train, 
                                                      X_block_train,
                                                      X_extreme_train,
