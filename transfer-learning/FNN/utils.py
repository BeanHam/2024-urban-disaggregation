...
def load_data(low, high, parameters):
    ...
    ## load data
    X_low_train = torch.from_numpy(np.load(source_low_path+'_train.npy')).float()[-portion:]
    X_low_test = torch.from_numpy(np.load(source_low_path+'_test.npy')).float()
    X_high_train = torch.from_numpy(np.load(source_high_path+'_train.npy')).float()[-portion:]
    X_high_test = torch.from_numpy(np.load(source_high_path+'_test.npy')).float()
    X_low_train, X_low_val, X_high_train, X_high_val = train_test_split(X_low_train, X_high_train, test_size=0.2, random_state=1)
